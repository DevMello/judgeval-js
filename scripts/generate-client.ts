...
function generateClient(methods: MethodInfo[]): string {
  const lines = [
    `// Total wall-clock bound per request. Note: the Python SDK's`,
    `// httpx.Client(timeout=30) is per-phase (connect/read/write); an`,
    `// AbortSignal deadline is the closest fetch equivalent.`,
    `const REQUEST_TIMEOUT_MS = 30_000;`,
    ``,
    `/** Structured error for non-2xx Judgment API responses. */`,
    ...
    `      }`,
    `      throw new JudgmentAPIError(response.status, detail);`,
    `    }`,
...
function collectReferencedTypes(schema: any): Set<string> {
  ...
  function collect(value: any): void {
    ...
    if ("$ref" in value) {
      ...
      refs.add(toPascalCase(ref.replace("#/components/schemas/", "")));
    }
    if ("$id" in value && value !== schema) {
      refs.add(value.$id);
    }
    for (const v of Object.values(value)) collect(v);
  }
...
function generateModelFile(
  ...
  lines.push(generateInterface(name, schema, schemasById));
  lines.push(``);
...
async function main(): Promise<void> {
  ...
  const modelsDir = `${apiDir}/models`;
  fs.mkdirSync(modelsDir, { recursive: true });

  const schemaNames: string[] = [];

  for (const [schemaId, schema] of [...schemasById.entries()].sort()) {
    const modelCode = generateModelFile(schemaId, schema, schemasById);
    fs.writeFileSync(`${modelsDir}/${schemaId}.ts`, modelCode);
    schemaNames.push(schemaId);
    console.error(`Generated: ${modelsDir}/${schemaId}.ts`);
  }

  const modelsIndexCode = [
    `// Auto-generated by scripts/generate-client.ts`,
    `// DO NOT EDIT MANUALLY`,
    ``,
    ...schemaNames.map((name) => `export type { ${name} } from "./${name}";`),
    ``,
  ].join("\n");

  fs.writeFileSync(`${modelsDir}/index.ts`, modelsIndexCode);
  console.error(`Generated: ${modelsDir}/index.ts`);
